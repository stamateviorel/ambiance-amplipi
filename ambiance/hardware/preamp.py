...
# Vendored + slimmed from AmpliPi amplipi/rt.py (GPL-3.0-or-later, micro-nova/AmpliPi).
# Made standalone for ambiance-amplipi: the only app-level deps were the two volume
# constants and one alert helper, inlined below, so this needs ONLY smbus2 + pyserial
# + stdlib (no fastapi / pydantic / house.json closure).
MIN_VOL_DB = -80
MAX_VOL_DB = 0
  ...
  def update_zone_vol(self, zone, vol):
    """ Update the sources to all of the zones
      ...
      Args:
        ...
        vol: int in range[MIN_VOL_DB, MAX_VOL_DB]
    ...
    assert zone >= 0
    assert 0 <= preamp <= 5
    assert MIN_VOL_DB <= vol <= MAX_VOL_DB
    return True
